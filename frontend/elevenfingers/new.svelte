      ...
      switch(data.type) {
        ...
        case 'endGame':
          ...
          if (typingGameComponent) {
            typingGameComponent.endGame(data.results);
          }
          break;
        default:
        ...
        {#if gameResults}
          <div class="results">
            ...
            <ul>
              ...
              <li>Final position: {playerRanks[currentUserId] || sortedPlayers.findIndex(([playerId]) => playerId === currentUserId) + 1} of {sortedPlayers.length}</li>
            </ul>
          </div>
        {/if}
